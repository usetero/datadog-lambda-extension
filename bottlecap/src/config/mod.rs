...
use crate::config::policy_provider::{PolicyProviderConfig, deserialize_policy_providers};
...
pub struct LambdaConfig {
    ...
    /// Enable Tero policy-based filtering of telemetry. Default `false`.
...
pub struct LambdaConfigSource {
    ...
    /// `DD_POLICY_ENABLED` — enable Tero policy-based telemetry filtering.
    #[serde(deserialize_with = "deser_opt_bool")]
    pub policy_enabled: Option<bool>,
    /// `DD_POLICY_PROVIDERS` — JSON array (env) or list (YAML) of policy
    /// provider configs.
